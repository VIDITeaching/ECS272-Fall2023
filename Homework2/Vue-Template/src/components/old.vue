...
export default {
    ...
    methods: {
        ...
        initChart() {
            ...
            let yExtents = d3.extent(this.bars.map((d: jobTitleBar) => d.job_title_average_salary as number)) as [number, number]
            ...
            const bars = chartContainer.append('g')
                ...
                .attr('y', (d: jobTitleBar) => yScale(d.job_title_average_salary) as number)
                // specify the size of the rectangle
                .attr('width', xScale.bandwidth())
                .attr('height', (d: jobTitleBar) => Math.abs(yScale(0) - yScale(d.job_title_average_salary))) // this substraction is reversed so the result is non-negative
                .attr('fill', 'teal')
  ...
  export default {
    data() {
      return {
        ...
        margin: { left: 60, right: 20, top: 20, bottom: 80 } as Margin,
    ...
    methods: {
      ...
      initChart() {
        ...
        let yExtents = d3.extent(
          this.bars.map((d: jobTitleBar) => d.job_title_average_salary as number)
        ) as [number, number];
  
        let xCategories: string[] = [
          ...new Set(this.bars.map((d: jobTitleBar) => d.job_title as string)),
        ];
        ...
        const bars = chartContainer
          ...
          .attr("y", (d: jobTitleBar) => yScale(d.job_title_average_salary) as number)
          .attr("width", xScale.bandwidth())
          .attr("height", (d: jobTitleBar) =>
            Math.abs(yScale(0) - yScale(d.job_title_average_salary))
          )
          .attr("fill", (d: jobTitleBar, i: number) => this.colors(i));
        ...
        const title = chartContainer
          ...
          .style("text-anchor", "middle")
          .style("font-weight", "bold")
          .text("Bar Chart about Average Salary");
      },
    },
    watch: {
      rerender(newSize) {
